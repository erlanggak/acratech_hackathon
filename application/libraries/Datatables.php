...
class Datatables {
    ...
    private $where = array();
    private $default_where = array();
    private $default_having = array();
    ...
    public function having($column) {
        $this->having[] = $column;
        $this->ci->db->having($column, false);
        return $this;
    }

    /**
     * Generates the Having Wajib portion of the query
     *
     ...
     * @return mixed
     */
    public function default_having($column) {
        $this->default_having[] = $column;
        $this->ci->db->having($column, false);
        return $this;
    }
    ...
    /**
     * Generates the WHERE Wajib portion of the query
     *
     * @param mixed $key_condition
     * @param string $val
     * @param bool $backtick_protect
     * @return mixed
     */
    public function default_where($key_condition, $val = NULL, $backtick_protect = TRUE) {
        $this->default_where[] = array($key_condition, $val, $backtick_protect);
        $this->ci->db->where($key_condition, $val, $backtick_protect);
        return $this;
    }
     ...
     * @return mixed
     */
    public function like($key_condition, $val = NULL, $wildcard = 'both') {
        $this->like[] = array($key_condition, $val, $wildcard);
     ...
     * @return mixed
     */
    public function or_like($key_condition, $val = NULL, $wildcard = 'both') {
        $this->or_like[] = array($key_condition, $val, $wildcard);
        $this->ci->db->or_like($key_condition, $val, $wildcard);
        return $this;
    }
    ...
    private function get_display_result() {
        // printData($this->select);
        // printData($this->columns);

        $tes = $this->ci->db->get($this->table);
        // printData($this->ci->db->last_query());
        return $tes;
    }
    ...
    private function get_total_results($filtering = FALSE) {
        // printData($this->select,1);
        foreach ($this->select as $key => $value) {
            if ($key != $value) {
                // cek $key kalo ada _as_ ga usah
                // printData($key);
                // printData($value);
                $this->ci->db->select($value . " as " . $key, false);
            } else {
                $this->ci->db->select($key, false);
            }
        }
        ...
        foreach ($this->default_where as $val)
            $this->ci->db->where($val[0], $val[1], $val[2]);

        foreach ($this->default_having as $val)
            $this->ci->db->having($val);
